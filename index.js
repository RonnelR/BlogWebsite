...
app.get("/posts/:anotherPost",function(req,res){
  let newPost =  _.lowerCase(req.params.anotherPost);
  ...
  .then(function(result){
    ...
    {
      let Title =  _.lowerCase(title.heading);
  
      let bodyContent = title.bodycontent;
      
      if(newPost === Title){ 
       res.render("post",{Another : title.heading, Body : bodyContent}); 
     }
    });
  })
  .catch((err)=>console.log(err))
...
app.post("/",function(req,res){
  ...
  const headValue = req.body.ComposeValue;
  const bodyValue = req.body.forPost;
